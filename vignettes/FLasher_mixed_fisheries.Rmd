...
library(ggplotFL)
library(knitr)
opts_chunk$set(dev='png', cache=FALSE, fig.width=5, fig.height=4.5, tidy=TRUE, dpi=300, fig.pos='!h')
options(width=60)
```
...
```{r, load_om}
load("../data/mixed_fishery_example_om.RData")
```
...
bt3 <- FLFishery(solBT=flfs[["bt"]][["solBT"]])
bt3@effort[] <- 1
gn3 <- FLFishery(solGN=flfs[["gn"]][["solGN"]])
gn3@effort[] <- 1
