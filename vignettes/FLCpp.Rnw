...
\affil{European Commission, DG Joint Research Centre, Directorate D - Sustainable Resources, Unit D.02 Water and Marine Resources, via Enrico Fermi 2749, 21027 Ispra (VA), Italy}
\affil{Corresponding author: \href{mailto:finlay.scott@jrc.ec.europa.eu}{finlay.scott@jrc.ec.europa.eu}}
%\soundtrack{Car Seat Headrest}
...
This document describes the FLR classes and methods that have been implemented in C++ and are currently held in the \pkg{FLasher} package.
All classes can be used with either \class{double} or \class{CppAD::AD<double>} (defined as \class{adouble}) from the \pkg{CppAD} library.

An \pkg{Rcpp} plugin has been written for \pkg{FLasher}. This allows the C++ classes and methods inside \pkg{FLasher} to be used with the \pkg{Rcpp} \code{cppFunction()} function so that they can be easily used in a live R session. We use this a lot in this vignette.
Additionally, dedicated \code{wrap<>} and \code{as<>} methods have been written for most of the classes to allow them to be easily passed to and from C++ and R.
...
The C++ based \class{FLQuant} class is basically the same as the R version.
Internally, the data member of the C++ class is represented as \code{std::vector<T>}, i.e. it is a one dimensional vector not a 6D array like the R version.
A range of accessing methods are available that allow the C++ version to be treated as a 6D array.

The C++ \class{FLQuant} has been templated, which means that it can store different types of data.
At the moment, the data type of a C++ \class{FLQuant} (\code{T}) is either a \code{double} or \code{CppAD::AD<double>} (defined as \class{adouble}).
The \class{adouble} type is for automatic differentiation using the \pkg{CppAD} library.
There are \code{typedef}s for \class{FLQuant<T>}: just \class{FLQuant} for \class{double} and \class{FLQuantAD} for \class{adouble}. 

Making the class{FLQuant} class work for other data types, e.g. other implementations of automatic differentiation, is possible.

The C++ \class{FLQuant} also has members for units, dimensions and dimension names, like the R version.


\subsection{Moving \class{FLQuant}s between R and C++}

Here we write a simple C++ function, in an R session, that takes an \class{FLQuant}, multiplies it by a \class{double} and returns the result (more on mathematical operators on C++ \class{FLQuant} objects in Section~\ref{sec:flqmath}).
First we write the C++ source code as a text string that we store as an R variable.
...
Note that because dedicated \code{wrap<>} and \code{as<>} methods have been written, the function directly takes and returns an \pkg{FLQuant}.

We then compile the source code with \code{cppFunction}, specifying the we depend on \pkg{FLasher} and that we need to use C++11.

<<compile_simple_example>>=
cppFunction(code=source, depends = 'FLasher', plugins='cpp11', rebuild=TRUE)
@

This makes the C++ function \code{flq\_test()} available to R. To test it we need an \class{FLQuant} in our R session. We'll use the catch numbers from the \code{ple4} data:
...
We can now call the \code{flq\_test()} function:
...
Something happened. What object was returned? We can use the \pkg{testthat} package to compare the input and output.

<<examine_flq_test>>=
# What is it
is(flq_out)
# Has the multiplication worked? Look at a subset
flq_in[1:3,"2000"]
flq_out[1:3,"2000"]
# Use testthat
# The following test only produces output if the test fails
expect_equal(flq_in * mult, flq_out)
@
...
The method \code{get\_size()} returns the size of the \class{FLQuant} as a single integer.
...
They are of limited use in C++ world.

\subsection{Accessing the data in an \class{FLQuant} in C++}
\label{sec:flquantaccess}

This is the important section: how to actually get at the data in the \class{FLQuant}. It is possible to pull out the data member directly (the \code{std::vector<T>}). However, you cannot operate directly on it. You have to use the accessors. This is to ensure that the size of the vector remains correct.

Note that ALL accessors work on indices, i.e. positions within the dimension, NOT dimension names.
For example, if the year range of the \class{FLQuant} is 2000 to 2010, to access year 2005, the year index is 6.
Names do not work with the accessors, only integers.

\subsubsection{Extracting data}

There are several accessor methods to get at the data. These either return single values of type \class{T} or \class{FLQuant<T>}.
Note that the indices start at 1, not 0, which follows R but is contrary to C++.

\begin{table}[H]
    \centering
    \begin{tabu} to \textwidth {  X[l2]  X[l5]  X[l3]  }
        \hline
        Method & Syntax & Notes\\  
        \hline
        \multirow{3}{0.2\textwidth}{Get single value (returns type \class{T}) }& \code{flq(n)} & Accesses the nth element of the data member vector, starting from 1. \\
                                           & \code{flq(quant, year, unit, season, area, iter)} & Similar to R version. Indices start at 1.\\
                                           & \code{flq(std::vector)} & Vector of \class{unsigned int} of length 6, corresponding to quant, year etc. \\
        \hline
        \multirow{3}{0.2\textwidth}{Get subset (returns type \class{FLQuant<T>})} &  \code{flq(quant\_min, quant\_max, year\_min, year\_max, ..., iter\_min, iter\_max)} & Results in long code. \\
                                                                & \code{flq(indices\_min, indices\_max)} & \code{indices} are \class{std::vector<unsigned int>} of length 6 to specify the range. \\
                                                                & \code{flq(quant, year, unit, season, area)} & Returns all iterations. \\
        \hline
    \end{tabu}
    \caption{Available data extraction methods for C++ \class{FLQuant} objects. Note that indices start at 1, not 0, which follows R but contrary to C++.}
    \label{tab:flqextract}
\end{table}


This function demonstrates ways of extracting single values:

<<flq_accessors>>=
source = '
    std::vector<double> flq_extract1(FLQuant x, unsigned int n,
        std::vector<unsigned int> indices){
        std::vector<double> out {
            // Single element accessor (starting from 1)
            x(n),
            // 6D index as individual arguments (starting from 1)
            x(indices[0],indices[1],indices[2],indices[3],indices[4],indices[5]),
            // 6D index as single vector (starting from 1)
            x(indices)
        };
        return out;
    }
...
<<test_flq_accessors1>>=
n <- 20
indices <- c(2,3,1,1,1,1)
out <- flq_extract1(flq_in, n, indices)
out
# Check
expect_equal(c(flq_in)[n], out[1])
expect_equal(c(flq_in[indices[1], indices[2], indices[3], indices[4],
                      indices[5], indices[6]]), out[2])
expect_equal(c(flq_in[indices[1], indices[2], indices[3], indices[4],
                      indices[5], indices[6]]), out[3])
@

This function demonstrates ways of extracting subsets of an \class{FLQuant}. It introduces a new class \class{FLQuant7} which is just a list of \class{FLQuant} objects (see Section~\ref{sec:flq7}):

<<flq_subset_accessors>>=
source = '
    FLQuant7 flq_extract2(FLQuant x, std::vector<unsigned int> indices_min,
        std::vector<unsigned int> indices_max){
        // Empty return list
        FLQuant7 out;
        // Long faffy way to extract the subset
        out(x(indices_min[0], indices_max[0], indices_min[1], indices_max[1],
            indices_min[2], indices_max[2], indices_min[3], indices_max[3],
            indices_min[4], indices_max[4], indices_min[5], indices_max[5]));
        // Neater way
        out(x(indices_min, indices_max));
        // All iters
        out(x(indices_min[0], indices_min[1], indices_min[2], indices_min[3], indices_min[4]));
        return out;
    }
'
cppFunction(code=source, depends = 'FLasher', plugins='cpp11', rebuild=TRUE)
...
<<test_flq_accessors2>>=
# Add some iters to the flq
flq_in <- propagate(flq_in, 20)
# Indices for subsetting
indices_min <- c(2,3,1,1,1,5)
indices_max <- c(7,10,1,1,1,10)
out <- flq_extract2(flq_in, indices_min, indices_max)
# What is out
is(out)
length(out)
# Check indices accessor
expect_equal(flq_in[indices_min[1]:indices_max[1], indices_min[2]:indices_max[2],
                    indices_min[3]:indices_max[3], indices_min[4]:indices_max[4],
                    indices_min[5]:indices_max[5], indices_min[6]:indices_max[6]], out[[1]])
# Vector accessor
expect_equal(flq_in[indices_min[1]:indices_max[1], indices_min[2]:indices_max[2],
                    indices_min[3]:indices_max[3], indices_min[4]:indices_max[4],
                    indices_min[5]:indices_max[5], indices_min[6]:indices_max[6]], out[[2]])
# All iters
expect_equal(flq_in[indices_min[1], indices_min[2], indices_min[3], indices_min[4], indices_min[5], ],
                    out[[3]])
@

\subsubsection{Inserting data}

There are several ways of inserting data into an \class{FLQuant}.
...
\begin{table}[H]
    ...
    \begin{tabu} to \textwidth {  X[l2]  X[l5]  X[l3]  }
        ...
        Method & Syntax & Notes\\  
        \hline
        \multirow{3}{0.2\textwidth}{Insert single value}& \code{flq(n)=x} & Inserts \code{x} at the the nth element of the data member vector, starting from 1. \\
                                           & \code{flq(quant, year, unit, season, area, iter)=x} & Similar to R version. Indices start at 1.\\
                                           & \code{flq(std::vector)=x} & Vector of \class{unsigned int} of length 6, corresponding to quant, year etc. \\
        \hline
        Insert an \class{FLQuant} of same or smaller dimension. & \code{flq.insert(flq\_in, indices\_min, indices\_max)} & Uses \code{insert()} method. Need to specify where \code{flq\_in} is being inserted into \code{flq}.\\
        \hline
        Fill an \class{FLQuant} with the same value & \code{flq.fill(x)} & \code{x} is of type \class{T}.\\
        \hline
    \end{tabu}
    \caption{Available data insertion methods for C++ \class{FLQuant} objects. Note that indices start at 1, not 0, which follows R but contrary to C++.}
    \label{tab:flqextract}
\end{table}

As mentioned above, note that the indices for inserting into and accessing the \class{FLQuant} start at 1, even though we are in C++ world (\class{std::vector<>} etc. obviously start at 0).

Some examples:

<<flq_subset_accessors>>=
source = '
    FLQuant7 flq_insert(FLQuant x, FLQuant y, std::vector<unsigned int> indices_min, std::vector<unsigned int> indices_max, unsigned int n, double value){
        // Empty return list
        FLQuant7 out;
        ...
        FLQuant x2 = x;
        x2(indices_min[0], indices_min[1], indices_min[2], indices_min[3], indices_min[4], indices_min[5])=value;
        out(x2);
        // Insert a single value at a position using a vector
        FLQuant x3=x;
        x3(indices_min)=value;
        out(x3);
        // Insert an entire FLQuant
        FLQuant x4=x;
        x4.insert(y, indices_min, indices_max);
        out(x4);
        // Fill the entire FLQuant with the the same value
        FLQuant x5=x;
        x5.fill(value);
        out(x5);
        return out;
...
indices_min <- c(2,3,1,1,1,5)
indices_max <- c(7,10,1,1,1,10)
# Make a mini FLQ based on stock.n
mini_flq <- propagate(stock.n(ple4),20)
mini_flq <- mini_flq[indices_min[1]:indices_max[1], indices_min[2]:indices_max[2],
                    indices_min[3]:indices_max[3], indices_min[4]:indices_max[4],
                    indices_min[5]:indices_max[5], indices_min[6]:indices_max[6]]
n <- 15
value <- 666
out <- flq_insert(flq_in, mini_flq, indices_min, indices_max, n, value)
# Check indices insert
expect_equal(c(out[[1]])[n], value)
# All dims inserter
expect_equal(c(out[[2]][indices_min[1], indices_min[2],
                    indices_min[3], indices_min[4],
                    indices_min[5], indices_min[6]]), value)
# Vector inserter
expect_equal(c(out[[3]][indices_min[1], indices_min[2],
                    indices_min[3], indices_min[4],
                    indices_min[5], indices_min[6]]), value)
# FLQuant inserter
expect_equal(out[[4]][indices_min[1]:indices_max[1], indices_min[2]:indices_max[2],
                    indices_min[3]:indices_max[3], indices_min[4]:indices_max[4],
                    indices_min[5]:indices_max[5], indices_min[6]:indices_max[6]], mini_flq)
# Fill
expect_equal(c(out[[5]]), rep(value, prod(dim(flq_in))))
@
...
\section{Lists of \class{FLQuant} objects: \class{FLQuant7}}
\label{sec:flq7}

Here we introduce the class \class{FLQuant7}. This is just a list of \class{FLQuant} objects.
...
%The residuals can be accessed by the \code{get\_residuals()} method.
%
...
%Also, the method does not apply residuals, only deals with the deterministic output from the recruitment function.
%As such it is quite limited and it's direct use by users is not encouraged. Instead the \code{predict\_recruitment()} should be used.
%
%\subsection{Predicting recruitment}
%
%The \code{predict\_recruitment()} method evaluates the SR function for a series of SRP values (passed as an \class{FLQuant}) and applies the residuals (either multiplicative or additive, as determined by the \code{residuals\_mult} member).
%
%As well as taking an \class{FLQuant} of SRP values, the method also takes a \class{std::vector<unsigned in>} that specifies the initial index position of the required parameters and residuals. This is necessary because it is possible to pass in subsets of the full range of the SRP values and therefore we also need to subset the parameters and residuals appropriately. For example, if we have an operating model with an annual timestep and multiple iterations, we may want to predict recruitment for all iterations but only one of the timesteps (as would happen in the projection methods for an operating model). We would pass in a \class{FLQuant} of SRP values which only had one year and multiple iterations. However, to ensure that the residuals and SR parameters of that particular year were used, we also to tell the method which year we want. This is done using the \code{initial\_params\_indices} argument. It is an integer vector of length 5 that specifies the start positions of the year, unit, season, area and iter of the SR params and residuals to be applied.
%
%The result of \code{predict\_recruitment()} is an \class{FLQuant} of recruitment.
%
...
%
%\subsubsection{The \code{eval()} method}
%
%The \code{eval()} method is the method that evaluates the SR function with a given SRP and given SR parameters. It does not apply any residuals to the calculated recruitment.
%
...
%ssb <- c(ssb(ple4)[,1])
%# Eval the C++ function we wrote above for the first year, unit, season etc.
...
%
%In this example, the SR parameters are still fixed in time but the timestep of the model is seasonal with 4 seasons.
%The projection methods in \pkg{FLasher} evaluate the SR model \textit{at every time step}.
%This means that if the model has a seasonal timestep, recruitment will happen in every season. If the SR parameters are not disaggregated seasonally, the same SR parameter values will be used to calculate recruitment in every season and year.
%The stock will effectively recruit at every time step, using the same parameter values for each time step.
%This is probably not what you want and we deal with this below. 
%
%Before we get to that, here is an example to demonstrate this behaviour using the objects created above. We only have 1 season in the SR params and consequently this is used for all seasons that the SR mode is evaluated:
%
...
%@
%
%The effect, as seen above, is that the stock recruits in every season, which is probably not what you want.
%
%The way to control recruitment is through the SR parameters.
...
%
%If you use SR parameter indices that are greater than the dimensions of the SR parameters, then the index will be set to 1 in that dimension.
%For example, if we try to use season 5 of our parameters (which only have 4 seasons), the value for season will be used. An error will not be thrown,
%
%<<example_sr_params_recycling>>=
...
%
%To have the SR params change over time / unit / iter etc., simply set up the \class{FLQuant} of SR parameters to reflect that.
%For example, here we have SR parameter with an annual timestep, where the parameter values change each year.
%
%<<fwdsr_year_example>>=
...
%test_fwdSR_eval('ricker', year_params, residuals, residuals_mult, ssb, c(1, 1, 1, 1, 1))
%# Evaluate the value in year 2 with the same SSB
%test_fwdSR_eval('ricker', year_params, residuals, residuals_mult, ssb, c(2, 1, 1, 1, 1))
%# Evaluate the value in year 3 with the same SSB
%test_fwdSR_eval('ricker', year_params, residuals, residuals_mult, ssb, c(3, 1, 1, 1, 1))
%# And so on
%@
%
%This technique can be used when including an environmental covariate in the SR model.
%
...
%
%
%\subsubsection{Predicting recruitment}
%
%The above examples used the \code{eval\_model()} method. This method is limited because it only takes a single value of SRP and does not apply residuals.
%The \code{predict\_recruitment()} method gets round these limitations.
%
%Instead of passing a single value for SRP, you pass in an \class{FLQuant} of SRP. You also need to pass in a vector of integers that specify the initial indices of the residuals and SR parameters.
%
%In the following examples we always use multiplicative residuals. The residuals can be additive by setting the \code{residuals\_mult} member to \code{FALSE}.
%
%First we write a wrapper function that allows us to call the method:
%
%<<wrapper_for_predict_recruitment>>=
%source = '
%    FLQuant test_fwdSR_predict_recruitment(std::string model_name, const FLQuant params, const FLQuant residuals, const bool residuals_mult, const FLQuant srp, const std::vector<unsigned int> initial_param_indices){
%        fwdSR fwdsr(model_name, params, residuals, residuals_mult);
%        FLQuant rec = fwdsr.predict_recruitment(srp, initial_param_indices);
%        return rec;
%}
%'
%cppFunction(code=source, depends = 'FLasher', plugins='cpp11', rebuild=TRUE)
%@
%
%Now we generate some objects to test it. Here we want our SSB and residuals to have an annual timestep and multiple iterations. The SR parameters will be fixed in time and for all iterations.
%
%<<predict_rec_simple_objects>>=
%niters <- 100
%# Make our SSB with iterations - based on ple4 but with noise
%ssb_in <- propagate(ssb(ple4), iter=niters)
%ssb_in[] <- abs(rnorm(prod(dim(ssb_in)), mean=c(ssb(ple4)), sd=10))
%# Shorten it to make it neater
%ssb_in <- window(ssb_in, start=2000, end=2005)
%# Our parameters do not vary by time or have iterations - use the ones above
%params
%# Our residuals have the same dims as the SSB - here we just make up some random numbers
%# The residuals will be multiplicative
%residuals <- ssb_in
%residuals[] <- abs(rnorm(prod(dim(residuals)), mean=1, sd = 0.1))
%residuals
%@
%
%If we wanted the recruitment in all years and iterations, we pass in the objects, specifying that the initial indices are all 1 (the start of the residuals and parameters).
%The vector of initial indices is of length 5, not 6, because the length of the first dimension is always 1 (i.e. the parameters and residuals are not age structured).
%
%<<rec_all_years>>=
%rec <- test_fwdSR_predict_recruitment("ricker", params, residuals, TRUE, ssb_in, c(1, 1, 1, 1, 1))
%rec
%@
%
%If we wanted the recruitment in only one year, we pass in the objects to the method, specifying which year we want for the SR parameters and residuals.
%Here we get the recruitment in year 3 (accessed by index, not by name). The first value of the indices is therefore 3 (the year). The other indices are all 1 (we want all iterations so we start at 1).
%
%<<>>=
%rec <- test_fwdSR_predict_recruitment("ricker", params, residuals, TRUE, ssb_in[,3], c(3, 1, 1, 1, 1))
%rec
%@
%
%In this example we have a seasonal timestep and multiple iterations. First we make the objects.
%Remember that in the projection methods, recruitment happens in every timestep. Here we only want recruitment to happen in season 2. We therefore need to set the SR parameters in seasons 1, 3 and 4 to always give a recruitment value of 0.
%
%<<pred_rec_season_objects>>=
%niters <- 100
%# The SSB object with 4 seasons
%ssb_in <- FLQuant(NA, dim=c(1,dim(ssb(ple4))[2],1,4), iter=niters)
%ssb_in[] <- abs(rnorm(prod(dim(ssb_in)), mean=c(ssb(ple4)), sd=10))
%ssb_in <- window(ssb_in, start=1, end=5)
%ssb_in
%# The parameters
%params_season <- FLQuant(NA, dim=c(2,1,1,4), dimnames=list(param = c("a","b")), iter=niters)
...
%# Set seasons 1, 3, 4 to 0 so that we only reproduces in season 2
%season <- 2
%params_season[,,,!(1:4 %in% 2)] <- 0
%params_season
%# residuals must be same dim as SSB - even if seasonal
%# We also set the residuals to 0 apart from season 2 (only really necessary if additive residuals)
%residuals <- ssb_in
%residuals[] <- abs(rnorm(prod(dim(residuals)), mean=1, sd = 0.1))
%residuals[,,,!(1:4 %in% 2)] <- 0
%residuals
%@
%
%To get the recruitment for all years, seasons and iters, we can just pass in the objects and specify the initial indices to be 1 (the start)
%
%<<rec_season1>>=
%rec <- test_fwdSR_predict_recruitment("ricker", params_season, residuals, TRUE, ssb_in, c(1, 1, 1, 1, 1))
%rec
%@
%
%In a projection it is likely that we only want the recruitment for all iterations in one timestep. Here we get the recruitment in year 2, season 1.
%We subset the SSB and chance the initial indices accordingly.
%
%<<rec_season2>>=
%rec <- test_fwdSR_predict_recruitment("ricker", params_season, residuals, TRUE, ssb_in[,2,,1], c(2, 1, 1, 1, 1))
%rec
%@
%
%It is zero for all iterations (good, we don't recruit in season 1, only season 2).
%
%Here we get the recruitment in year 2, season 2.
%Again, we subset the SSB and chance the initial indices accordingly.
%
%<<rec_season3>>=
%rec <- test_fwdSR_predict_recruitment("ricker", params_season, residuals, TRUE, ssb_in[,2,,2], c(2, 1, 2, 1, 1))
%rec
%@
%
...
%recR <- params_season["a",,,2] * ssb_in[,2,,2] * exp(-params_season["b",,,2] * ssb_in[,2,,2])
%# Apply residuals
%recR <- recR * residuals[,2,,2]
%recR
%@
%
%Looks OK.
...
%%
%%<<fwdbiol_constructor, eval=FALSE>>=
%%source = '
