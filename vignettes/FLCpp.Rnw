...
\begin{itemize}
    ...
    \item the time lag between spawning and recruitment;
    \item the residuals;
...
The SR functions are declared at the bottom of the fwdSR.h script and defined at the bottom of the fwdSR.cpp script.
The model map is initialised in the \code{init\_model\_map()} method in fwdSR.cpp.
The arguments for each function are a single \class{T} value for SSB (where \class{T} is either a \class{double} or an \class{adouble}), and a \class{std::vector<double} for the parameters (of length equal to the number of parameters). The function returns a single \class{T} value of recruitment.
...
At the time of writing only two SR functions have been written: \code{ricker()} and \code{bevholt()}.

To add a new SR function you have to do three things:

\begin{enumerate}
    \item Write a new function declaration in fwdSR.h. The function should have the arguments as described above (see existing functions for structure).
    \item Write the function code in fwdSR.cpp (again, see existing functions for structure).
    \item Add the new function to the model map by adding lines to the \code{init\_model\_map()} in fwdSR.cpp. The right-hand side is the name of your new function. The left-hand side is the character string that your model will be called by the user in \R (copy the existing style).
\end{enumerate}
...
The parameters are stored as an \class{FLQuant}. The first dimension is used for the parameters. The other dimensions allow the parameters to vary over year, unit, season, area and iter if necessary. If the parameters are constant in that dimension, for example if the parameters do not change with year or season, then those dimensions have length 1.

In the simplest case, the \code{alpha} and \code{beta} parameters for the Ricker SR function are constant over year, unit, season, area and iter and so would be stored in an \class{FLQuant} with dimensions (2,1,1,1,1,1).
If the parameters do vary with one of the dimensions 2 to 5, they should have the same length as that dimension in the model. Otherwise they should have length 1.
For example, if your SR parameters vary with season and there are 4 seasons in the model, the parameter \class{FLQuant} should have a length of 4 in the season dimension.

Note that the order of the parameters in the first dimension is the order they are passed into the SR function.

The SR parameters are passed in from \R as an \class{FLQuant} which means that if they are being taken from an \class{FLSR} object they will need to be converted from an \class{FLPar} class first. NOTE: after developing this class, I added a function to convert \class{FLPar} objects to \class{FLQuant} objects (used by \class{FLCatch}). This code could be used to pass in the SR parameters directly as an \class{FLPar} without the need for additional conversion.

\subsection{Evaluating the SR function with the SR parameters}

To evaluate the SR function the \code{eval\_model()} method is used. This takes a single value of the SSB (as either a \class{double} or an \class{adouble}) plus indices for the year to iter dimension. The dimension indices are passed in so that the correct SR params are used. A single value of recruitment, of the same type as SSB, is returned.

Note that the indices start from 1 and cannot be passed as names. For example, if your SR parameters varied with year and had a year range of 2000 to 2010, to evaluate the recruitment in year 2005, the year argument to \code{eval\_model()} would be 6.

The \code{eval\_model()} method only processes one SSB value at a time.
It does not apply residuals which are dealt with after \code{eval\_model()} has been called.

\subsection{Residuals}

Residuals are stored as an \class{FLQuant}. No check is made that the residuals are the right size (i.e. are the same size as the other objects in your operating model) but if you try to access the residuals outside of their range then it will fail nicely with a message.
The residuals are accessed by the \code{get\_residuals()} method.

The residuals are not applied by the \code{eval\_model()}. They are just there for you to use them as you want. The same is true for the member which stores if the residuals are multiplicative or not (accessed throught the \code{get\_residuals\_mult()} method).
...
It may be better for the residuals to be applied in the \code{eval\_model()} method, but this method was really designed to \textit{only} evaluate the SR function, i.e. get the deterministic recruitment.

\subsection{Creating and using a \class{fwdSR}}
\label{sec:usingfwdSR}

Here we give some simple examples that illustrate the \class{fwdSR} class works.
However, it is not particularly useful to use the class outside of an operating model.

There is no implict \code{as<>} or \code{wrap<>} code for the \class{fwdSR} class because the class does not exist in FLR, only in the C++ code (Note: you could write an \code{as<>} that based on a \class{list} of components but I didn't see the advantage as you will still have to compile the list of components yourself. Same goes for \code{wrap<>}. We don't need to return an \class{fwdSR} object to \R, except for testing, so there was no point in writing \code{wrap<>}).
...
First we write a simple wrapper function to the \class{fwdSR} constructor in C++ and evaluate the model with the passed in value of SSB at a particular time step, unit etc.
...
The \code{project\_timestep()} method in \pkg{FLasher} evaluates the SR model \textit{at every time step}.
...
Here is an example to demonstrate this using the objects created above. We only have 1 season in the SR params and consequently this is used for all seasons that the SR mode is evaluated:
...
If your stock only recruits in 1 season (or twice a year etc.), you must set the SR parameters to have seasons, and set the values of the parameters to give the required recruitment.
...
So now even though \code{project\_timestep()} evaluates the SR function at every timestep, the effective recruitment in the timesteps where the stock does not recruit is 0.
