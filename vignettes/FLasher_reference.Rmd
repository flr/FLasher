...
The dimensions of the SRR parameter *FLQuant* do not need to match the dimension of the `fwdBiol` as values are recycled where necessary.
...
In this first example we have a simple annual model with a first age of 1.
...
Annual - fixed parameters

Annual - changing parameters

Seasonal - one unit - no trend
...
The model can become complicated when the stock has multiple units due to multiple recruitment events.
...
A stock with multiple recruitment events must have a unit for each recruitment event and must also be seasonal (recruitment can only happen once in a timestep).

As mentioned above, due to recycling of SRR parameters, it is necessary to disaggregate the SRR parameters by unit and season.
...
For example, consider a seasonal model which has 4 seasons and recruitment happens in seasons 2 and 4.
The `spwn` slot of the *fwdBiol* should have been set up so that the SRP in seasons 2 and 4 is 0 (so recruitment is 0 in seasons 1 and 3).
