...
A stock may spawn and recruit more than one time in a year, e.g. it can recruit in multiple seasons, but recruitment can only happen once in a timestep.
...
The calculated recruitment is inserted into the abundance in the same timestep as we calculate it meaning that we calculate recruitment at the start of the timestep.
...
Different target types require different combinations of Fishery, Catch and Biol to be specifed (NA means not specified). The following table describes the requirements:

Fishery | Catch | Biol | Notes
------- | ----- | ---- | -----
        |       |      | Error - you have to specify something
        |       |   x  | Total catch, landings, discards or Fbar of a Biol (where possible to calculate - see below). Biological targets e.g. SSB
        |   x   |      | Error - if you specify a Catch you must also specify a Fishery
        |   x   |   x  | Error - if you specify a Catch you must also specify a Fishery
   x    |       |      | Total revenue or effort of a fishery
   x    |       |   x  | Error - also needs Catch to be specified
   x    |   x   |      | Catch, landings, discards or revenue of a particular Fishery / Catch
   x    |   x   |   x  | Partial F of a Fishery / Catch on a Biol

### Fishery targets

Specify Fishery only - no Catch or Biol 

Total Revenue - calculated as...

Effort - taken from object

### Catch targets

Must specify Catch and Fishery number, OR, a Biol number (not both)
...
* `SSB` is the mass of mature fish (sum N * wt * mat) `at the end of the timestep`, i.e. the survivors after all mortality (fishing and natural) has stopped.
* `Biomass` is the total mass of fish (sum N * wt) `at the end of the timestep`, i.e. the survivors after all mortality (fishing and natural) has stopped.
...
All 6 dimensions are set, including the age range (from the `minAge` and `maxAge` column in the control object).
Only some target types need an age range (such as fbar). However, it is always set because the target types are not treated differently by `get_target_value_hat()`.
The indices range is not set by target type.
