...
A fishery is made up of a series of catches, each one is represented by an object of class `FLCatch`. It is possible for a fishery to have only a single catch.
...
An `FLCatch` represents catches from a single species (usually from a single biological stock - mixing catches from more than one stock is a special case where multiple biological stocks are represented as catches from a single stock).
...
IS THIS RIGHT?
EFFORT SHARE?
WHAT ABOUT MULTIPLE PASSIVE GEARS DEPLOYED IN PARALLEL?

Costs are at vessel level (fixed costs) but what about vessels that use multiple gears.
During a year each gear does not use the same effort. But effort is defined at the fishery level.
So we consider each gear as a seperate fishery? But then your costs are split between two fishery objects for same vessel.

Or is fishery 'fishing activity' - so a fishing activity can catch multiple species.
But then how do we get economics in there - can get costs and revenues from an activity - but only at activity level

An `FLCatch` can be used to represent fishing activity at different levels of disaggregation.
For example, it can be used to represent the activity of a specifc metier or gear, or an aggregations of metiers or gears.
...
The biological stocks are projected one timestep by the `project_biols()` method.
The method calculates the survivors from the previous timestep and places them in the correct position age class in the current timestep.
Recruitment is also calculated.

The stocks are processed one at a time (the order in which they appear in the `biols` method).
The stocks are independent of each other and there is no interaction between them.

The basic process is:

- Calculate total mortality $z$ on the stock in the previous timestep.
- Calculate survivors for the current timestep.
...
where $N$ is the population abundance. Here we calculate the survivors from the previous timestep.

Where the survivors are placed in the current timestep depends on the season and the timing of recruitment.
...
The same thing happens for seasons 1 and 2 in year 2.
In season 3 in year 2, the survivors are placed in the next age group, i.e. the individuals are now a year older.
