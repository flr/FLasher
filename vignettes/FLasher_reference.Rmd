...
A `fwdBiol` can be fished by multiple `FLCatch`es.
...
The total fishing mortality imposed on a stock from all the `FLCatches` that fish it is the sum of the partial fishing mortality.
...
The value of $\alpha$ is recycled over the dimensions over which it does not change.
