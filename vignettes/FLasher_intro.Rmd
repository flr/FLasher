...
__FLasher__ is an FLR package for performing projections through time.
It is a sequel to an older package __FLash__ that has been widely used for projecting **FLStock** objects.
...
* The [tutorial](http://www.flr-project.org/doc/Forecasting_on_the_Medium_Term_for_advice_using_FLasher.html) on running medium term projections with an **FLStock**.
* The package vignette *Mixed fisheries with FLasher*, for running projections with **FLBiol(s)** and **FLFishery(ies)** objects,

More details of the internal workings of __FLasher__ can be found in the vignette *FLasher reference manual*.
There is an additional vignette on using the __FLasher__ plugin for accessing automatic differentiation capabilities in R and other nerdy things.
