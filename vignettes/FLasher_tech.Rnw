...
As can deal with multiple fisheries and stocks, it can also solve multiple targets at the same time.
...
target1 <- data.frame(year = 2000,
                     quantity = rep('catch',2),
                     value = rep(1000,2),
                     min = NA,
                     fishery = c(1,2),
                     catch = c(1,1),
...
# Same as first target but now with biomass constraint
target2 <- data.frame(year = c(2001, 2001, 2002),
...
target_iters <- array(NA, dim=c(nrow(target),3,5), dimnames=list(target_no=1:nrow(target),
                                                                 c("min","value","max"),
                                                                 iter=1:5))
target_iters[1:4,'value',] <- rnorm(20,mean=1000,sd=1)
target_iters[5,'min',] <- rnorm(5,mean=10000,sd=10)
...
test_fwdControl_get_target_row(fc_in, 2, 2)
# Pull out all targets from an integer column
test_fwdControl_get_target_int_col(fc_in, 1, 'year')
test_fwdControl_get_target_int_col(fc_in, 2, 'fishery')
...
test_fwdControl_get_target_value2(fc_in, 2, 3, 1) # min of sim target 3 from target 2
...
test_fwdControl_get_target_effort_timestep(fc_in, 2, 1) # catch in timestep 2
test_fwdControl_get_target_effort_timestep(fc_in, 2, 3) # biomass in timestep 3
@
