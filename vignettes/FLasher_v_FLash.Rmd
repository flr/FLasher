...
SEPARATE DOCUMENT NOT A VIGNETTE AS ALSO NEEDS FLASH.
AND FLASH AND FLASHER NEED TO BE LOADED IN PARTICULAR ORDER
...
flasher_stf <- FLasher::fwd(ple4_stf, control=flasher_ctrl, sr = list(model = model, params=params(ple4.sr)))
```
...
```{r}
c(landings_target, discards_max)
landings(flash_stf)[,ac(2009:2011)]
discards(flash_stf)[,ac(2009:2011)]
landings(flasher_stf)[,ac(2009:2011)]
discards(flasher_stf)[,ac(2009:2011)]
...
flasher_ctrl_target <- data.frame(year = rep(2009:2011, each=2),
                        ...
                        relSeason = c(1, NA, 1, NA, 1, NA),
                        relBiol = c(1, NA, 1, NA, 1, NA),
                        quant = rep(c("landings", "discards"), 3),
...
```{r }
stock.n(ple4p_stf) <- rnorm(niters, mean=stock.n(ple4_stf), sd=100)
stock.wt(ple4p_stf) <- rnorm(niters, mean=stock.wt(ple4_stf), sd=0.01)
landings.wt(ple4p_stf) <- rnorm(niters, mean=landings.wt(ple4_stf), sd=0.01)
discards.n(ple4p_stf) <- rnorm(niters, mean=discards.wt(ple4_stf), sd=0.01)
```

We project for 3 years with a simple landings target (the same target value for all iterations).
...
flasher_ctrl_target <- data.frame(year = 2009:2011,
                        ...
                        value = landings_target)
flasher_ctrl <- FLasher::fwdControl(flasher_ctrl_target)
flasher_stf <- FLasher::fwd(ple4p_stf, control=flasher_ctrl, sr = list(model = model, params=params(ple4.sr)))
```

Have the targets been hit in the projection years 2009 to 2011?
All landings iterations have hit the target.
...
However, due to the stochasticity in the original *FLStock* object, there is stochasticity in the projected abundances:
...
FIX THE RESIDUALS IN FLASHER FWD
...
#flasher_stf <- FLasher::fwd(ple4p_stf, control=flasher_ctrl, sr = list(model = model, params=params(ple4.sr)), residuals=residuals)
```
...
flasher_stf <- FLasher::fwd(ple4p_stf, control=flasher_ctrl, sr = list(model = model, params=sr_params))
```
...
#flasher_stf <- FLasher::fwd(ple4p_stf, control=flasher_ctrl, sr = list(model = model, params=sr_params), sr.residuals=residuals, sr.residuals.mult=TRUE)
```
