...
In this example we attempt to decrease F by the same amount each year but we also constrain this by the amount landings is allowed to change each year by adding relative min and max limits.
...
```{r }
new_n <- stock.n(ple4p_stf)
new_n <- new_n * rnorm(prod(dim(new_n)), mean=1, sd=0.1)
stock.n(ple4p_stf) <- new_n

new_wt <- landings.wt(ple4p_stf)
new_wt <- new_wt * rnorm(prod(dim(new_wt)), mean=1, sd=0.1)
landings.wt(ple4p_stf) <- new_wt

new_wt <- discards.wt(ple4p_stf)
new_wt <- new_wt * rnorm(prod(dim(new_wt)), mean=1, sd=0.1)
discards.wt(ple4p_stf) <- new_wt
```
...
flash_ctrl_target <- data.frame(year = 2009:2011,
			  quantity = "landings",
			  val = landings_target)
flash_ctrl <- FLash::fwdControl(flash_ctrl_target, iters=niters)
...
This is a complicated example where we have a relative F target with a maximum allowable change in the landings.

all sources of stochasticity, plus
